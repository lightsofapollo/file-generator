 ...
 *
 * type: A javascript type like Boolean, Number or an array of values 
 *      (string by default)
 *
 * defaultValue: Default value for prompt
 *
...
Question.prototype = {
   ...
   * @param {Function} callback
   */
  confirmPrompt: function(callback){
    this.simplePrompt(callback, 'confirm');
  },
   ...
   * @param {Function} callback
   */
  simplePrompt: function(callback, promptType){
    var self = this;
    ...
    program[promptType](this.prompt, function(value){
      value = self._filterValue(self._defaultValue(value));
      callback(value);
    });
